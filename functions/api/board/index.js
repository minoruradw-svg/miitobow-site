...
const MAX_IMAGES = 20;
// 動画は画像よりファイルサイズが大きく容量を圧迫しやすいため、保持件数は少なめ（直近5件）にする。
// 1件あたりの上限はKVの1件25MBハード上限に対して安全マージンを取り、実体（デコード後）で
// 24MBまで許容する（2026-09-11・当初15MB前後にしていたが実運用で不足が判明し引き上げ）。
// base64は元データの約4/3に膨らむため、文字列長の上限はその分大きく取る。
const MAX_VIDEO_DATA_LENGTH = 32 * 1024 * 1024;
const MAX_VIDEOS = 5;
